...
export class EventsService {
  ...
  async findOne(id: number): Promise<Event> {
    ...
    if (!event) throw new NotFoundException('Event nije pronadjen');
    return event;
  }
  ...
  async apply(eventId: number, applicant: User, message?: string, teamId?: number): Promise<Application> {
    ...
    if (existingApplication) {
      throw new BadRequestException('Vec ste se prijavili na ovaj event');
    }
  ...
  async updateApplicationStatus(applicationId: number, status: string): Promise<Application> {
    ...
    const statusText = status === 'accepted' ? 'prihvacena' : 'odbijena';
    await this.notificationsService.create(
